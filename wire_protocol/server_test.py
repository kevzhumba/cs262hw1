...
class ServerTest(unittest.TestCase):
    ...
    def test_create_account_success(self):
        ...
        response = self.server.process_create_account(args, joseph_socket, threading.Lock())
    ...
    def test_create_account_fail_logged_in(self):
        ...
        response = self.server.process_create_account(args, joseph_socket, lock)
        self.assertEqual(response['status'], 'Error: User can\'t create an account while logged in.')
    ...
    def test_login_success(self):
        ...
        mock_kevin_lock = MagicMock()
        mock_kevin_socket = MagicMock()
        response = self.server.process_login(args, mock_kevin_socket, mock_kevin_lock)
        self.assertEqual(response['status'], 'Success')
    ...
    def test_login_fail_doesnt_exist(self):
        args = {"username": "joseph"}
        joseph_socket = MagicMock()
        response = self.server.process_login(args, joseph_socket, threading.Lock())
    ...
    def test_login_fail_someone_logged_in(self):
        ...
        mock_kevin_socket = MagicMock()
        response = self.server.process_login(args, mock_kevin_socket, mock_kevin_lock)
        self.assertEqual(response['status'], 'Error: Someone else is logged into that account.')
    ...
    def test_send_msg_success(self):
        ...
        response = self.server.process_send_msg(args, self.server.logged_in['kevin'][0], self.server.logged_in['kevin'][1])
        self.assertEqual(response['status'], 'Success')
    ...
    def test_send_msg_failure_no_recipient(self):
        ...
        response = self.server.process_send_msg(args, self.server.logged_in['kevin'][0], self.server.logged_in['kevin'][1])
        self.assertEqual(response['status'], 'Error: The recipient of the message does not exist.')

    def test_delete_account_success(self):
        response = self.server.process_delete_account(self.server.logged_in['kevin'][0], self.server.logged_in['kevin'][1])
        self.assertEqual(response['status'], 'Success')
        self.assertFalse('kevin' in self.server.account_list)
    
    def test_logoff_success(self):
        response = self.server.process_logoff(self.server.logged_in['kevin'][0], self.server.logged_in['kevin'][1])
        self.assertEqual(response['status'], 'Success')
